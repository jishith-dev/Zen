...
int _httpServer_listen(HttpServer *server) {
  ...
  int opt = 1;
#ifdef _WIN32
  setsockopt(server->serverFd, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt,
             sizeof(opt));
#else
...
static char *readUntilHeadersEnd(socket_t fd, size_t *outLen,
                                 size_t *headerEnd) {
...
static int findHeaderValue(const char *headerBlock, size_t headerBlockLen,
                           const char *headerName, char *outVal,
                           size_t outValCap) {
...
static void appendResponseHeader(HttpRequest *req, const char *name,
                                 const char *value) {
  ...
  int n =
      snprintf(req->headers + req->headersLen,
               HEADER_BUF_SIZE - req->headersLen, "%s: %s\r\n", name, value);
...
static void sendResponse(HttpRequest *req, int statusOverrideOrNeg,
  ...
  int n = snprintf(header, sizeof(header),
                   ...
                   : (status >= 400 && status < 500) ? "Client Error"
                                                     : "Server Error",
                   contentType, req->headers, bodyLen);
  ...
  if (n > 0 && (size_t)n < sizeof(header)) {
    ...
    int fn = snprintf(fallback, sizeof(fallback),
                      ...
                      status);
    if (fn > 0)
      send(req->clientFd, fallback, (size_t)fn, 0);
    bodyLen = 0;
  }
...
HttpRequest *_httpServer_next(HttpServer *server) {
  ...
  if (contentLength > 0) {
    ...
    while (have < contentLength) {
      int n =
          recv(req->clientFd, bodyBuf + have, (int)(contentLength - have), 0);
      if (n <= 0) {
#ifndef _WIN32
        if (n < 0 && errno == EINTR)
          continue;
...
void _httpRequest_setHeader(HttpRequest *req, const char *name,
                            const char *value) {
...
void _httpRequest_sendFile(HttpRequest *req, const char *path,
  ...
  sendResponse(req, -1, contentType ? contentType : "application/octet-stream",
               buf, readBytes);

  free(buf);
}

const char *_httpRequest_method(HttpRequest *req) {
  return (req && req->method) ? req->method : "";
}
const char *_httpRequest_path(HttpRequest *req) {
  return (req && req->path) ? req->path : "";
}
const char *_httpRequest_body(HttpRequest *req) {
