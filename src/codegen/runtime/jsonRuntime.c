...
static void parse_fail(JsonParser *p, const char *what) {
    char buf[256];
    snprintf(buf, sizeof(buf), "Invalid JSON: expected %s at position %zu", what, p->pos);
    zen_error("JsonError", buf);
}

static char *parse_raw_string(JsonParser *p) {
    if (p->src[p->pos] != '"') parse_fail(p, "'\"'");
    p->pos++;
    ...
    size_t len = 0;
    char *buf = malloc(cap);
    if (!buf) zen_error("MemoryError", "Failed to allocate memory for Json string");
    ...
    while (p->pos < p->len && p->src[p->pos] != '"') {
        ...
        if (c == '\\') {
            p->pos++;
            if (p->pos >= p->len) parse_fail(p, "escape sequence");
            ...
            switch (esc) {
                ...
                case 'f': out = '\f'; break;
                case 'u': {
                    if (p->pos + 4 >= p->len) parse_fail(p, "unicode escape");
                    char hex[5] = {
            ...
            if (len + 1 >= cap) {
                ...
                if (!tmp) zen_error("MemoryError", "Failed to allocate memory for Json string");
                buf = tmp;
            }
            buf[len++] = out;
            p->pos++;
    ...
    if (p->pos >= p->len) parse_fail(p, "closing '\"'");
    p->pos++;
...
static ZenJson *parse_number(JsonParser *p) {
    ...
    if (!numbuf) zen_error("MemoryError", "Failed to allocate memory for Json number");
...
static ZenJson *parse_array(JsonParser *p) {
    ...
    while (1) {
        ...
        if (j->as.arr.count + 1 >= j->as.arr.cap) {
            ...
            ZenJson **tmp = realloc(j->as.arr.items, j->as.arr.cap * sizeof(ZenJson *));
            if (!tmp) zen_error("MemoryError", "Failed to allocate memory for Json array");
            j->as.arr.items = tmp;
        }
        j->as.arr.items[j->as.arr.count++] = item;

        skip_ws(p);
        if (p->pos >= p->len) parse_fail(p, "',' or ']'");
...
static ZenJson *parse_object(JsonParser *p) {
    ...
    while (1) {
        skip_ws(p);
        if (p->pos >= p->len || p->src[p->pos] != '"') parse_fail(p, "string key");
        char *key = parse_raw_string(p);

        skip_ws(p);
        if (p->pos >= p->len || p->src[p->pos] != ':') parse_fail(p, "':'");
        p->pos++;
        ...
        if (j->as.obj.count + 1 >= j->as.obj.cap) {
            ...
            ZenJson **vtmp = realloc(j->as.obj.values, j->as.obj.cap * sizeof(ZenJson *));
            if (!ktmp || !vtmp) zen_error("MemoryError", "Failed to allocate memory for Json object");
            j->as.obj.keys = ktmp;
...
static ZenJson *parse_value(JsonParser *p) {
    skip_ws(p);
    if (p->pos >= p->len) parse_fail(p, "value");
...
ZenJson *_zen_json_parse(const char *str) {
    if (!str) zen_error("JsonError", "Cannot parse null string as Json");

    JsonParser p = { str, 0, strlen(str) };
    ZenJson *result = parse_value(&p);

    skip_ws(&p);
    if (p.pos != p.len) zen_error("JsonError", "Trailing data after Json value");
...
static ZenJson *json_lookup(ZenJson *obj, const char *key) {
    if (obj->type != ZEN_JSON_OBJECT) {
        char buf[256];
        snprintf(buf, sizeof(buf), "Cannot get key '%s': value is not a Json object", key);
        zen_error("JsonError", buf);
    }
...
static void expect_type(ZenJson *j, ZenJsonType want, const char *key, const char *typeName) {
    if (j->type != want) {
        char buf[256];
...
char *_zen_json_getString(ZenJson *obj, const char *key) {
    ...
    if (!copy) zen_error("MemoryError", "Failed to allocate memory for Json string");
...
int _zen_json_has(ZenJson *obj, const char *key) {
    ...
    for (size_t i = 0; i < obj->as.obj.count; i++) {
        if (strcmp(obj->as.obj.keys[i], key) == 0) return 1;
    }
    return 0;
}
...
static ZenJson *array_at(ZenJson *arr, int index) {
    ...
    if (index < 0 || (size_t)index >= arr->as.arr.count) {
        char buf[256];
        snprintf(buf, sizeof(buf), "Json array index %d out of bounds (length %zu)", index, arr->as.arr.count);
        zen_error("JsonError", buf);
    }
