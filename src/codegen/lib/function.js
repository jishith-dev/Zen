export class HandleFunction {
  ...
  handleFunction(node, isInline = false) {
    ...
    for (const p of paramData) {
      if (p.isList) {
        ...
        this.IRB.functionParamTable.set(p.name, {
          ...
          isThread: false,
          ownerFunction: this.IRB.anonymCurrentFunction
            ? this.IRB.anonymCurrentFunction.name
            : this.IRB.currentFunction?.name,
        });
      } else {
        let ptr = `%${p.name}.addr`;
    ...
    functionState.body.splice(2, 0, this.IRB.allocaBuff.join("\n"));
