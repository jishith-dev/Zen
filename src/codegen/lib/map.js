export class ZenMap {
  ...
  handleMap(node, globalScope) {
    ...
    this.IRB.declareOneTime("zen_map_new", "declare ptr @_zen_map_new()");
    ...
    if (node.value.type !== "MAP_LITERAL") {
      ...
      if (!expr.isMap) {
        this.IRB.emitError(
          "TypeError",
          `Map ${name} expected Map but got ${expr.type}`,
          node,
        );
      }
    ...
    const layout = this.IRB.buildMapLayout(node.value);

    this.IRB.maps.set(name, layout);

    // CREATE ROOT MAP

    const mapPtr = this.IRB.newTemp();

    if (globalScope) {
      this.IRB.globals.push(`${gName} = global ptr null`);
      this.IRB.emit(`${mapPtr} = call ptr @_zen_map_new()`);
    ...
    this.IRB.buildMapRecursive(mapPtr, node.value);
  }
}
