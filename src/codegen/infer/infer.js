    ...
    switch (node.type) {
      ...
      case "CALL": {
        ...
        if (node.callee && node.callee.type === "MEMBER_ACCESS") {
          ...
          // infer object type
          const structName = this.infer(object);

          const fullMethodName = `${structName}_${methodName}`;
      ...
      case "BINARY_EXPRESSION": {
        ...
        if (["&&", "||"].includes(op)) {
          if (leftType !== "bool") {
            this.IRB.emitError(
              "TypeError",
              `Logical operator '${op}' requires operands of type 'bool'`,
              node,
            );
          }
      ...
      case "UNARY_EXPRESSION": {
        ...
        if (node.operator === "-") {
          let orgType = this.ensureNumeric(valueType, "-", node);
